...
在 C++ 中，可以使用延迟循环来暂停程序的执行一段时间。实现延迟循环的方法有多种，常见的有使用标准库函数 `std::this_thread::sleep_for`，以及利用系统特定的函数，比如 Windows 下的 `Sleep` 函数或 Unix 系统下的 `usleep` 函数。

##### 1. 使用 `std::this_thread::sleep_for`

C++11 引入了 `<thread>` 头文件中的 `std::this_thread::sleep_for` 函数，它可以让当前线程休眠指定的时间。
...
在这个示例中，`std::this_thread::sleep_for` 使当前线程休眠 1 秒钟，每次循环都会延迟 1 秒后再执行下一次迭代。

##### 2. 使用平台特定的函数

######在 Windows 系统上

Windows 提供了 `Sleep` 函数（在 `<windows.h>` 头文件中），它接受毫秒为单位的延迟时间。
...
Unix 提供了 `usleep` 函数（在 `<unistd.h>` 头文件中），它接受微秒为单位的延迟时间。
...
int main() {
    for (int i = 0; i < 5; ++i) {
        std::cout << "Iteration " << i + 1 << std::endl;
        usleep(1000000); // 延迟 1000000 微秒（1 秒）
    }
...
==推荐使用 `std::this_thread::sleep_for`==，因为它是 C++ 标准库的一部分，跨平台且语义清晰。平台特定的函数 `Sleep` 和 `usleep` 也可以实现延迟，但它们依赖于特定的操作系统。选择哪种方法取决于你的项目需求和目标平台。

***

### 5.3 do while 循环

`do-while` 循环是 C++ 中的一种循环控制结构，与 `while` 循环类似，但有一个关键的区别：`do-while` 循环会先执行循环体，然后再检查条件。因此，无论条件是否为真，循环体至少会执行一次。
...
1. **至少执行一次**：由于条件检查在循环体之后进行，因此**循环体至少会执行一次**。
2. **条件检查在循环体后**：在执行完循环体后，检查条件，如果条件为真，继续执行循环体，否则退出循环。

`do-while` 循环<u>适用于那些需要在循环前至少执行一次代码的场景。例如，在用户输入验证中，程序至少需要提示用户输入一次</u>。
...
在这个示例中，无论用户第一次输入的值是否在 1 到 10 之间，程序都会提示用户输入，直到用户输入的值符合要求为止。
